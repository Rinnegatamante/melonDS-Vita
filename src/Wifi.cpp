...
bool CheckRX(bool block)
{
    ...
    *(u16*)&RXBuffer[10] = 0x4080; // min/max RSSI. dunno
    ...
    u16 addr = IOPORT(W_RXBumelon_fwriteCursor) << 1;
        ...
        if (!(RXTime & RXHalfwordTimeMask))
        {
            ...
            *(u16*)&RAM[addr] = *(u16*)&RXBuffer[RXBufferPtr];
            ...
            if (RXTime == 0) // finished receiving
            {
                ...
                // copy the RX header
                u16 headeraddr = IOPORT(W_RXBumelon_fwriteCursor) << 1;
                ...
                *(u16*)&RAM[headeraddr] = *(u16*)&RXBuffer[10];
                ...
                IOPORT(W_RXBumelon_fwriteCursor) = (addr & ~0x3) >> 1;
...
void Write(u32 addr, u16 val)
{
    ...
    case W_ModeReset:
        {
            ...
            if (val & 0x2000)
            {
                IOPORT(W_RXBumelon_fwriteAddr) = 0;
    ...
    case W_RXCnt:
        if (val & 0x0001)
        {
            IOPORT(W_RXBumelon_fwriteCursor) = IOPORT(W_RXBumelon_fwriteAddr);
        }
        if (val & 0x0080)
        {
            IOPORT(W_TXSlotReply2) = IOPORT(W_TXSlotReply1);
    ...
    case W_TXBufDataWrite:
        {
            u32 wraddr = IOPORT(W_TXBumelon_fwriteAddr);
            *(u16*)&RAM[wraddr] = val;
            ...
            IOPORT(W_TXBumelon_fwriteAddr) = wraddr & 0x1FFE;
